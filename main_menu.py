...
def main_menu():
    ...
    while m: # while loop  says when m.
        for event in pygame.event.get(): #fetching the items from event queue
            ...
            if buttons.side_button("Play Music", mouse[0], mouse[1], 1166, 75, 200, 50, constant.purple_color, constant.blue_color, 25):
...
def creditation():
    while True:
        ...
        if buttons.centre_button("Back", mouse[0], mouse[1], constant.WIDTH / 2 - 100, 700, 200, 50, constant.red_color, constant.white_color, 25, 20):
            main_menu()
